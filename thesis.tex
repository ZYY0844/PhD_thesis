...
\usepackage[headheight=14.5pt,inner=3.8cm,outer=2.5cm]{geometry} % good for print
% \usepackage[headheight=14.5pt,inner=3.1cm,outer=3.1cm]{geometry}
...
\begin{figure}[tb] 
    \centering 
    \includegraphics[width=1\columnwidth]{HP_pipline.pdf}
...
\end{figure}
\section{Data Augmentation for Spectrogram}\label{sec:data_augm}
\subsection{Overview} 
The overview of the proposed Horcrux is depicted in Figure~\ref{fig:HP_pipline} with two parallel modules:
\begin{itemize}
...
Signal decomposition is widely used in signal processing to identify components with different features and help the downstream algorithm extract target information e.g., ICA decomposes the signal from different sources based on statistical independence, and EMD decomposes the signal based on the oscillatory modes with different time scales~\cite{chen2022contactless}. Considering the nature of the cardiac activities, the radar signal reflected from the chest region reveals two characteristics on the spectrogram: (a) the stable components along certain frequency bands that represent either the heart rate frequency (near $1$Hz) or the center frequency of the prominent vibrations ($10-25$ Hz)~\cite{zhang2024radarODE-MTL}; (b) the transient components simply indicates the pulsatile heartbeats with large energy. In this research, these two components are named harmonic and percussive components (features) as highlighted in the red boxes in Figure~\ref{fig:HP_pipline}.
...
In this paper, the performance of the ECG recovery and heartbeat detection are evaluated from the following perspectives:
\begin{itemize}
...
\begin{figure}[tbp]
        \centering
        \subfloat[]{\label{fig:eg_0}\includegraphics[width=0.45\columnwidth]{synthetic_signal_3.pdf}}
        \subfloat[]{\label{fig:eg_1}\includegraphics[width=0.45\columnwidth]{synthetic_signal_0.pdf}}\\
        \subfloat[]{\label{fig:eg_2}\includegraphics[width=0.45\columnwidth]{synthetic_signal_2.pdf}}
        \subfloat[]{\label{fig:eg_3}\includegraphics[width=0.45\columnwidth]{synthetic_signal_1.pdf}}\\
        \subfloat[]{\label{fig:eg_4}\includegraphics[width=0.45\columnwidth]{synthetic_signal_5.pdf}}
        \subfloat[]{\label{fig:eg_5}\includegraphics[width=0.45\columnwidth]{synthetic_signal_6.pdf}}
...
\begin{figure}[tb] 
    \centering 
    \includegraphics[width=0.7\columnwidth]{performance.pdf}
