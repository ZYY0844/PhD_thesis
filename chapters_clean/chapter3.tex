...
\Gls{radar} system is originally designed for military detection of large aircraft by emitting electromagnetic waves and evaluating the reflections. The follow-up research has investigated the civilian use of radar systems for contactless sensing in various scenarios, such as autonomous driving~\cite{yao2023waterscenes,bilik2019rise,wang2021exploring,sun2020deep,meyer2019automotive,li2020lidar,feng2020deep,caesar2020nuscenes,wang2021rodnet,palffy2020cnn,kim2020multi,nobis2019deep,zhang2021radar,svenningsson2021radar,guan2020through,scheiner2020seeing,li2021lidar,kim2020low,yin2021radar,barnes2020oxford,wang2021rethinking,kim2021deep,lim2019radar,sless2019road,major2019vehicle,mostajabi2020high,long2021radar,ouaknine2021carrada,gao2021ramp,yang2020radarnet,xu2021rpfa} and human monitoring~\cite{sasakawa2019human, sakamoto2017radar, kim2016hand, amin2019hand, zhang2016dynamic, ritchie2017hand, li2018dynamic, zhang2018latern, zhao2023low, sun2021smart, wang2024rodar, lien2016soli, chen2018micro, singh2019radhar, wang2021joint, yang2020mm, li2019robust, wang2016phase, sun2020gesture, yang2020portable, wang2021g, chen2021mGesture, wang2022radar, ma2019signfi, li2020wireless, chen2022radar, zhang2018multimodal, yang2025g3r, amin2018radar, wang2023radhar}. Over the past decade, radar-based sensing has been empowered by deep neural networks to process non-stationary reflected signals or high-dimensional data, enabling versatile applications to replace contact- or visual-based measurement for convenience or privacy concerns (e.g., vital sign monitoring~\cite{liu2024diversity}, gesture recognition~\cite{song2025dual}, fault diagnosis~\cite{chen2024two}).
...
\begin{figure}[tb]
        \centering
        \subfloat[]{\label{fig:radar_good}\includegraphics[width=0.35\columnwidth]{radar_good.eps}}
        \subfloat[]{\label{fig:radar_bad}\includegraphics[width=0.35\columnwidth]{radar_bad.eps}} 
        \caption{\textcolor{black}{Challenges for high-SNR signal collection: (a) and (b) Radar signals with high and low SNR for adjacent points with a distance of $0.03$m. The ECG ground truth is plotted to help the identification of the prominent peaks.}}
        \label{fig:challenges}
\end{figure}
Based on the discussion Chapter~\ref{sec:the_first_challenge}, it is still a challenge to precisely locate and track the cardiac location during data collection to efficiently extract high-SNR radar signal. \textcolor{black}{Although, it is natural to think the high-SNR radar signal can be searched in a constrained space by optimization, there is no appropriate method to assess the signal SNR in terms of cardiac features contained, and the objective space is actually highly discontinuous because the adjacent points may reveal totally different SNR as shown in Figure~\subref*{fig:radar_good} and~\subref*{fig:radar_bad}, with high-SNR radar signal shown prominent peaks aligning with ECG peaks but not vice versa.} To address these issues, the contributions of this study can be listed as: 
\begin{itemize}
...
\item \textcolor{black}{The rough location acts as the initial state for the CFT algorithm, and the points within a constrained space will be evaluated to find the red CF point with high SNR while ignoring the space where only low-SNR signals\footnote[2]{\textcolor{black}{There is no boundary for distinguishing high or low signals, because explicit values will be used as the accurate estimation of SNR instead of the vague binary classification.}} can be extracted from, as shown in Figure~\ref{fig:CFTRFcardi}(b).}
...
\begin{figure}[tb]
  \centering
  \subfloat[]{\label{fig:target_org}\includegraphics[width=0.4\columnwidth]{target_org.eps}}
  \subfloat[]{\label{fig:target_sig}\includegraphics[width=0.4\columnwidth]{target_sig.eps}} \\
  \subfloat[]{\label{fig:target_org_bad}\includegraphics[width=0.4\columnwidth]{target_org_bad.eps}}
  \subfloat[]{\label{fig:target_sig_bad}\includegraphics[width=0.4\columnwidth]{target_sig_bad.eps}}
...
The dataset contains a total of $80$-minute synchronous radar-ECG pairs collected for $5$ healthy subjects ($3$ men, $2$ women) in $2$ indoor scenarios as shown in Figure~\ref{fig:data_col}. \textcolor{black}{The subjects are asked to sit casually and are allowed to change postures during data collection, and each data trial lasts for $1$ minute. The distance between radar and human body varies from $0.5-1.2$m, and a longer distance causes the decrease of signal SNR with a smaller portion of the space points containing useful cardiac features.} 

\textcolor{black}{TI-AWR 1843 radar with $2$ Tx and $4$ Rx is used for data collection with $8$ virtual antenna channels created~\cite{AWR1843}, and the radar configurations are listed in Table~\ref{tab:data_param} with the name provided in TI mmWave-Studio interface. The signal will be sampled at $200$Hz, and only a band-pass filter from $0.5$ to $50$Hz and a differentiator are used for removing respiration noise because the radar signal extracted from CF points already has high SNR. Lastly, the ECG ground truth is collected using TI ADS1292 with AC coupling and integrated right-leg drive (RLD) amplifier to remove potential baseline drift or power-line noise, as shown in Figure~\ref{fig:data_col}. The related ECG processing (e.g., smoothing and peak finding) is realized by NeuroKit2 python package~\cite{makowski2021neurokit2} after the collection.}

\begin{figure}[tb]
  \centering
  \includegraphics[width=0.8\columnwidth]{scen_2.pdf}
  \caption{\textcolor{black}{Indoor scenarios for synchronous radar and ECG collection.}}
  \label{fig:data_col}
\end{figure}
...
\subsubsection{Deep Learning Model Training and Testing}
\textcolor{black}{The deep learning model adopts the same backbone, encoder and decoder structure with the same hyperparameters as used in our previous open-sourced work~\cite{zhang2024radarODE-MTL} coded in PyTorch and trained on NVIDIA RTX 4090 (24GB). The total training epoch is set to $100$ with batch size $8$, and each experiment is repeated for five times to ensure the statistical significance. All the results show the statistical significance with P values much smaller than $0.05$, hence is not shown in the tables or figures for clarity. The dataset is split based on a $5$-fold cross-validation training strategy, with the trials from 1 fixed subject for testing and the other 4 subjects alternatively selected for training or validation, ensuring to make use of all the possible trials while not involving the testing data in the training phase.}
...
\end{minipage}\begin{minipage}{0.9\linewidth}\centering
  \subfloat[]{\label{fig:bf_good}\includegraphics[width=0.3\columnwidth]{bf_good.eps}}
  \subfloat[]{\label{fig:MMECG_good}\includegraphics[width=0.3\columnwidth]{mmecg_good.eps}}
  \subfloat[]{\label{fig:vimo_good}\includegraphics[width=0.3\columnwidth]{devimo_good.eps}}\\
\end{minipage}
\begin{minipage}{0.1\linewidth}\centering\scriptsize
\rotatebox[origin=center]{0}{\textbf{Rough Loc.}}
...
\rotatebox[origin=center]{0}{\textbf{CF Point}}
\end{minipage}\begin{minipage}{0.9\linewidth}\centering
  \subfloat[]{\label{fig:bf_bad}\includegraphics[width=0.3\columnwidth]{bf_bad.eps}}
  \subfloat[]{\label{fig:MMECG_bad}\includegraphics[width=0.3\columnwidth]{mmecg_bad.eps}}
  \subfloat[]{\label{fig:vimo_bad}\includegraphics[width=0.3\columnwidth]{devimo_bad.eps}}
\end{minipage}
...
\begin{figure}[tb]
  \centering
  \subfloat[]{\label{fig:pk_err_point}\includegraphics[width=0.35\columnwidth]{pk_err_point.eps}}
  \subfloat[]{\label{fig:pk_mdr_point}\includegraphics[width=0.34\columnwidth]{pk_mdr_point.eps}} \\
  \subfloat[]{\label{fig:pk_err_cdf}\includegraphics[width=0.35\columnwidth]{pk_err_cdf.eps}}
  \subfloat[]{\label{fig:pk_mdr_cdf}\includegraphics[width=0.35\columnwidth]{pk_mdr_cdf.eps}} 
...
\begin{figure}[tb]
  \centering
  \subfloat[]{\label{fig:radar_clean}\includegraphics[width=0.4\columnwidth]{radar_clean.eps}}
  \subfloat[]{\label{fig:ecg_pred_clean}\includegraphics[width=0.4\columnwidth]{ecg_pred_clean.eps}} \\
  \subfloat[]{\label{fig:radar_noise}\includegraphics[width=0.4\columnwidth]{radar_noise.eps}}
  \subfloat[]{\label{fig:ecg_pred_noise}\includegraphics[width=0.4\columnwidth]{ecg_pred_noise.eps}} 
